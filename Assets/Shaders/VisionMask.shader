// 시야 마스크 — "밝게 비춰진 곳(손전등) = 선명 / 어두운 곳 = 검은 블러" 화면 후처리.
// 손전등 SpotLight의 실시간 그림자가 이미 벽 차폐를 계산하므로 별도 시야 판정 없이
// 픽셀 밝기(luminance)만으로 은폐 여부를 결정한다. VisionMaskOverlay가 메인 카메라
// 프러스텀을 채우는 쿼드에 이 셰이더를 입혀 사용. URP Opaque Texture 필수.
Shader "AmsalGame/VisionMask"
{
    Properties
    {
        _VisibleLumMin("이 밝기 이하 = 완전 은폐", Float) = 0.12
        _VisibleLumMax("이 밝기 이상 = 완전 선명", Float) = 0.30
        _BlurRadiusPx("은폐 블러 반경(px)", Float) = 12
        _HiddenBrightness("은폐 영역 밝기 배율", Range(0, 2)) = 0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                float vis = smoothstep(_VisibleLumMin, _VisibleLumMax, max(lum, blurLum));
